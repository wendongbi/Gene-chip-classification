    ...
    for turn in range(1):
        ...
        with tf.name_scope('loss'):
            with tf.name_scope('regularization_loss'):
                regularization_loss = 0.5 * tf.reduce_sum(tf.square(W1))
            ...
            with tf.name_scope('cross_entropy'):
                layer_loss = tf.reduce_sum(tf.maximum(tf.zeros([batch, labelNum]), 1 - y_ * y_output))
