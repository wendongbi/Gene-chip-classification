...
# Filename: logistic_Muti_diseasestate.py
...
if __name__ == '__main__':
    ...
    for turn in range(1):
        ...
        with tf.name_scope('regular_loss'):
        	regular_loss = regularization_loss = 0.5 * tf.reduce_sum(tf.abs(W1))
        with tf.name_scope('lr_loss'):            #loss = tf.reduce_sum(tf.maximum(tf.zeros([batch, labelNum]), 1 - y_ * y_output))
            lr_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=y_output,
                                                                      labels=y_))
            #svm_loss = tf.reduce_mean(tf.reduce_sum(tf.square(W1))/(2 *y_ * y_output))
            #tf.summary.scalar('svm_loss', loss)
        with tf.name_scope('loss'):
        	loss = regular_loss + 0.9 * lr_loss
